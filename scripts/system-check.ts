...
export function readNodeExecutableVersion(
  ...
  if (result.error) {
    return {
      ok: false,
      detail: `Unable to run \`node --version\`: ${result.error.message}. Weo requires Node.js ${MIN_NODE_ENGINE_RANGE} on PATH.`,
    }
  }
  ...
  if (result.status !== 0) {
    ...
    return {
      ok: false,
      detail: `Unable to run \`node --version\`${suffix}. Weo requires Node.js ${MIN_NODE_ENGINE_RANGE} on PATH.`,
    }
  }
  ...
  if (!version) {
    return {
      ok: false,
      detail: `Unable to read Node.js version from \`node --version\`. Weo requires Node.js ${MIN_NODE_ENGINE_RANGE} on PATH.`,
    }
  }
